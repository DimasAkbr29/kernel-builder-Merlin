...
# zy cromer clang
ZY_REPO="https://api.github.com/repos/ZyCromerZ/Clang/releases/latest"

show_usage() {
    CLANG_NAME="slim, rv, aosp, yuki, lilium, zy"
...
"lilium")
    ...
    ;;
"zy")
    curl -s "$ZY_REPO" | grep "browser_download_url" | grep ".tar.gz" | cut -d '"' -f 4
    ;;
*)
    if [[ -z $1 ]]; then
        show_usage
